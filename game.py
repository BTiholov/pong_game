...
class Game:
    ...
    def tick(self):
        if self.points_a > 9 or self.points_b > 9:
            self.game_over = True
            return
    ...
    def __perform_paddle_hit_checking(self):
        ...
        hit_paddle_a = a_x == (x - self.paddle_width) and ((a_y - self.paddle_height / 2) <= y <= (a_y + self.paddle_height / 2))

        b_x, b_y = self.paddle_b_pos
        hit_paddle_b = b_x == (x + self.paddle_width) and ((b_y - self.paddle_height / 2) <= y <= (b_y + self.paddle_height / 2))
